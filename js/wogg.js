...
class WoggSpider extends Spider {
  constructor() {
    super();
    this.siteUrl = "https://wogg.xxooo.cf";
  ...
  async parseVodShortListFromDoc($) {
    ...
    for (const item of items) {
      ...
      vodShort.vod_pic = $(item)
        .find(".module-item-cover .module-item-pic img")
        .first()
        .attr("data-src");
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.type_name =
      this.woggTypeObj[
        $('[class="video-info-aux"]').find("a")[0].attribs.title
      ];
    ...
    vodDetail.vod_remarks = `${
      $(video_items[3]).find("div")[0].children[0].data
    }, 制作人:Jade`;
    vodDetail.vod_content = $(video_items[4]).find("p")[0].children[0].data;

    vodDetail.vod_content = vodDetail.vod_content
      .replace("[收起部分]", "")
      .replace("[展开全部]", "");
  ...
  async parseVodShortListFromDocBySearch($) {
    ...
    for (const item of items) {
      ...
      vodShort.vod_pic = $(item).find(".module-item-pic > img")[0].attribs[
        "data-src"
      ];
  ...
  async setClasses() {
    ...
    if (!_.isEmpty(con)) {
      ...
      for (const element of elements) {
        let type_id = parseInt(
          element.attribs.href.split("/").slice(-1)[0].split(".html")[0]
        );
        let type_name = element.children
          .slice(-1)[0]
          .data.replace("\n", "")
          .replace(" ", "")
          .replace("玩偶", "");
  ...
  async setFilterObj() {
    for (const type_dic of this.classes) {
      ...
      if (type_id !== "/" && type_id !== "最近更新") {
        let url =
          this.siteUrl + `/index.php/vodshow/${type_id}--------1---.html`;
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let reqUrl =
      this.siteUrl + "/index.php/vodshow/" + urlParams.join("-") + ".html";
    let html = await this.fetch(reqUrl, null, this.getHeader());
    if (!_.isEmpty(html)) {
      let $ = load(html);
      this.vodList = await this.parseVodShortListFromDoc($);
      let total = Utils.getStrByRegex(
        /\$\("\.mac_total"\)\.text\('(\d+)'\)/,
        html
      );
      this.limit = 72;
  ...
  async setSearch(wd, quick) {
    let searchUrl =
      this.siteUrl + "/index.php/vodsearch/-------------.html?wd=" + wd;
